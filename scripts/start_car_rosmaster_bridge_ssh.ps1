...
$StartScript = "cd $(Quote-Bash($RemoteDir)) && nohup python3 $(Quote-Bash($RemoteBridgeFile)) --host 0.0.0.0 --port $Port --speed $Speed </dev/null > /tmp/icar_rosmaster_tcp_bridge.log 2>&1 & disown || true; exit 0"
